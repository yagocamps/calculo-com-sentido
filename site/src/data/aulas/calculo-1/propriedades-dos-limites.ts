...
export const propriedadesDosLimites: CurriculumLessonSpec = {
  ...
  explanation: [
    "Hipóteses comuns: considere funções reais definidas perto de um ponto de acumulação a do domínio, talvez sem valor no próprio ponto. Na mesma aproximação \\(x\\to a\\), suponha que \\(f(x)\\to L\\) e \\(g(x)\\to M\\), com L e M números reais finitos. As constantes c e k não variam com x.",
    "Ler \\(f(x)\\to L\\) significa: os valores de f ficam tão próximos de L quanto desejarmos quando x se aproxima suficientemente de a, com x diferente de a. Não precisamos conhecer \\(f(a)\\). Aplicamos as regras no domínio comum das expressões.",
    "As regras também valem em uma aproximação lateral, como \\(x\\to a\\) pela direita, ou quando \\(x\\to+\\infty\\) ou \\(x\\to-\\infty\\), desde que todos os limites usados sejam finitos e correspondam à mesma aproximação. Não combine um limite pela esquerda com outro pela direita. O limite bilateral exige concordância dos dois lados quando ambos fazem parte da aproximação considerada.",
  ],
  ...
  rules: [
    ...
    { id: "produto", title: "6. Produto", formulaLatex: "\\lim_{x\\to a}f(x)g(x)=LM", formulaAria: "O limite do produto de f e g é L vezes M.", conditions: "Cada fator deve ter limite finito. Um produto pode ter limite mesmo quando seus fatores não têm; nesse caso, esta regra não justifica separá-los.", example: "Com f tendendo a 2 e g a −3, fg tende a −6. Em contraste, x vezes \\(\\frac1x\\) vale 1 para \\(x\\ne0\\): em \\(x\\to0\\), não se pode calcular isso como zero vezes infinito." },
    { id: "quociente", title: "7. Quociente", formulaLatex: "\\lim_{x\\to a}\\frac{f(x)}{g(x)}=\\frac{L}{M},\\quad M\\ne0", formulaAria: "O limite de f dividido por g é L dividido por M, desde que M seja diferente de zero.", conditions: "\\(M\\ne0\\) garante que g não zera suficientemente perto de a no domínio considerado. Se \\(M=0\\), a regra não decide o limite: investigue sinais, laterais ou uma transformação algébrica.", example: "Em x→2, o numerador x+1 tende a 3 e o denominador x²+1 tende a 5, que não é zero. Portanto: \\[\\lim_{x\\to2}\\frac{x+1}{x^2+1}=\\frac35.\\] Já x/x em x→0 tem forma 0/0, mas simplifica para 1 com x≠0; 1/x tem laterais diferentes." },
    { id: "potencia", title: "8. Potências inteiras", formulaLatex: "\\lim_{x\\to a}[f(x)]^n=L^n", formulaAria: "O limite de f de x elevado a n é L elevado a n.", conditions: "Para n inteiro positivo, basta o limite finito de f. Para expoentes inteiros negativos, exija também \\(L\\ne0\\) e use o recíproco. Uma potência com expoente variável exige outra análise.", example: "Se f tende a −2, \\(f^3\\) tende a −8, enquanto \\(f^2\\) tende a 4. Como \\(-2\\ne0\\), f elevado a −2 tende a \\(\\frac14\\). Confira: \\((-2)^2=4\\), mas \\(-2^2=-4\\), pois sem parênteses a potência age antes do sinal." },
    { id: "raiz", title: "9. Raízes reais", formulaLatex: "\\lim_{x\\to a}\\sqrt[n]{f(x)}=\\sqrt[n]{L}", formulaAria: "O limite da raiz de índice n de f de x é a raiz de índice n de L.", conditions: "n é inteiro, \\(n\\ge2\\). Se n é ímpar, L pode ser qualquer real. Se n é par, exija \\(L\\ge0\\) e \\(f(x)\\ge0\\) nos pontos da aproximação em que usa a expressão; quando \\(L=0\\), essa condição perto do ponto não pode ser esquecida. Respeite o domínio e, quando necessário, use limite lateral.", example: "A raiz cúbica é real para números negativos: \\[\\lim_{x\\to-8}\\sqrt[3]{x}=-2.\\] Para a raiz quadrada na origem, consideramos x≥0: \\[\\lim_{x\\to0^+}\\sqrt{x}=0.\\] Saber apenas que f tende a 0 não garante que a raiz quadrada de f esteja definida nos dois lados: f(x)=x é um exemplo." },
    { id: "modulo", title: "10. Valor absoluto", formulaLatex: "\\lim_{x\\to a}|f(x)|=|L|", formulaAria: "O limite do módulo de f de x é o módulo de L.", conditions: "O limite finito de f garante o do módulo. O sentido inverso não vale em geral: o módulo perde a informação do sinal.", example: "Se f tende a −3, seu módulo tende a 3. Mas para \\(f(x)=\\frac{x}{|x|}\\), com \\(x\\ne0\\), o módulo é sempre 1 e tende a 1 em 0; f tem lateral −1 pela esquerda e 1 pela direita, portanto não tem limite bilateral." },
    { id: "composicao", title: "11. Composição com função contínua", formulaLatex: "\\lim_{x\\to a}h(f(x))=h(L)", formulaAria: "O limite da composição de h com f é h de L.", conditions: "Além de \\(f(x)\\to L\\), h deve ser contínua em L e a composição deve estar definida nos pontos considerados. Ter apenas um limite de h perto de L não basta se f assume exatamente L repetidas vezes.", example: "Se \\(f(x)=x^2+1\\) e \\(x\\to1\\), então f tende a 2. A raiz quadrada é contínua em 2, logo a raiz de \\(x^2+1\\) tende à raiz de 2. Para a condição importar: tome \\(f(x)=0\\) e \\(h(0)=7\\), mas \\(h(u)=1\\) para \\(u\\ne0\\). A composição vale 7; o limite de \\(h(u)\\) quando u→0 é 1, que não pode substituí-la." },
  ],
  appearances: [
    ...
    { label: "Comparação e Confronto", detail: "se \\(p\\le f\\le q\\) perto de a e p e q tendem ao mesmo L, f também tende a L" },
  ],
  exampleTitle: "Combinando regras com uma verificação de domínio",
  example: "Calcule o limite, quando \\(x\\to2\\), de \\(\\frac{3x^2-x+1}{\\sqrt{x+7}}\\). Antes de substituir, justifique as operações e a raiz.",
  steps: [
    { title: "Verificar a raiz", detail: "Perto de 2, \\(x+7\\) é positivo e tende a 9. Pela regra da raiz, \\(\\sqrt{x+7}\\) tende a 3." },
    { title: "Montar o numerador", detail: "Identidade: \\(x\\to2\\). Potência: \\(x^2\\to4\\). Múltiplo, diferença e constante: \\(3x^2-x+1\\to3\\cdot4-2+1=11\\)." },
  ...
  interpretation: [
    ...
    "Comparação preserva a ordem: se \\(f(x)\\le g(x)\\) perto de a e os limites são finitos, então \\(L\\le M\\). Uma desigualdade estrita pode virar igualdade no limite: para \\(x\\ne0\\), \\(0<x^2\\), mas ambos tendem a 0. Para cercar uma função com duas barreiras de mesmo limite, use o Teorema do Confronto, indicado nos próximos estudos.",
    "Limites infinitos não são números reais a inserir mecanicamente nas regras acima. As formas \\(\\infty-\\infty\\), \\(0\\cdot\\infty\\) e \\(\\frac\\infty\\infty\\) não determinam um valor; compare, por exemplo, \\(\\frac xx\\to1\\) e \\(\\frac{x^2}{x}\\to+\\infty\\) quando \\(x\\to+\\infty\\). As duas razões têm forma \\(\\frac\\infty\\infty\\) e resultados diferentes.",
  ],
  errors: [
    "Tratar limite de constante e múltiplo constante como a mesma regra.",
    "Dividir L por zero ou escrever \\(\\frac00\\) como resultado final.",
  ...
  guided: [
    { question: "Se \\(f\\to5\\) e \\(g\\to-2\\) na mesma aproximação, calcule a tendência de \\(3f-g+1\\).", hint: "Separe múltiplo, diferença e constante.", solution: "\\(3\\cdot5-(-2)+1=18\\).", answer: "18", interpretation: "O sinal de g precisa ser preservado na subtração." },
    { question: "Quando \\(x\\to1\\), calcule o limite de \\(\\frac{2x+1}{x^2+2}\\).", hint: "Confira se o limite do denominador é zero.", solution: "O numerador tende a 3 e o denominador também a 3. Como \\(3\\ne0\\), o limite é \\(\\frac33=1\\).", answer: "1", interpretation: "O quociente depende da hipótese do denominador." },
    { question: "Se \\(f\\to-2\\), qual o limite de f elevado a −3?", hint: "Escreva como o recíproco do cubo.", solution: "Como \\(-2\\ne0\\), usamos \\(\\frac1{(-2)^3}=-\\frac18\\).", answer: "-1/8", interpretation: "Expoente negativo exige limite da base diferente de zero." },
    { question: "Calcule o limite da raiz cúbica de x quando \\(x\\to-27\\).", hint: "Raiz de índice ímpar aceita radicando negativo.", solution: "O limite é a raiz cúbica de −27, que vale −3.", answer: "-3", interpretation: "Não aplique à raiz ímpar a restrição da raiz par." },
    { question: "Qual o limite de \\(\\sqrt x\\) quando \\(x\\to0\\) pela direita?", solution: "No domínio \\(x\\ge0\\), \\(\\sqrt x\\) tende a \\(\\sqrt0=0\\).", answer: "0", interpretation: "Na borda do domínio, declaramos o lado pelo qual nos aproximamos." },
    { type: "interpretacao", question: "Saber que o módulo de f tende a 1 garante que f tenha limite?", solution: "Não. Para \\(f(x)=\\frac{x}{|x|}\\) em \\(x\\to0\\), o módulo vale 1, mas os limites laterais de f são −1 e 1.", answer: "Não", interpretation: "O módulo elimina o sinal e pode esconder uma diferença entre os lados." },
    { type: "interpretacao", question: "Se o denominador tende a zero, podemos concluir que o quociente tende a infinito?", solution: "Não. \\(\\frac xx\\) tende a 1 em 0, \\(\\frac{x^2}{x}\\) tende a 0 e \\(\\frac1x\\) tem laterais infinitas de sinais diferentes. É necessário investigar a expressão.", answer: "Não", interpretation: "Denominador com limite zero é uma condição de análise, não um resultado." },
    { question: "Calcule o limite de \\(\\sqrt{2x^2+7}\\) quando \\(x\\to1\\).", hint: "Encontre primeiro o limite da expressão dentro da raiz.", solution: "\\(2x^2+7\\) tende a 9. É positivo perto de 1; a raiz é contínua em 9, então o resultado é 3.", answer: "3", interpretation: "A composição liga o limite da função interna à continuidade da externa." },
  ],
  ...
  summary: [
    ...
    "Constante, identidade, soma, diferença, múltiplo e produto formam a base; quociente exige \\(M\\ne0\\).",
