...
export const plots = {
  ...
  "leitura-completa-da-curva": {
    alt: "Curva que parte da altura dois, sobe até um pico na altura oito no meio do intervalo e desce de volta até dois.",
    x: [-0.4, 6.6], y: [0, 10],
    xTicks: [0, 1, 2, 3, 4, 5, 6], yTicks: [0, 2, 5, 8],
    legend: "Leitura completa: sobe de 0 a 3, desce de 3 a 6, com pico em (3, 8) e mesma altura nas duas pontas.",
    marks: [
      { kind: "curve", f: (x) => 2 + 6 * Math.sin((Math.PI * x) / 6), from: 0, to: 6, tone: "principal" },
      { kind: "point", at: [0, 2], tone: "aplicacao" },
      { kind: "point", at: [3, 8], tone: "alerta" },
      { kind: "point", at: [6, 2], tone: "aplicacao" },
      { kind: "text", at: [3, 9], text: "pico (3, 8)", tone: "alerta", anchor: "middle" },
      { kind: "text", at: [1.3, 3.9], text: "sobe", tone: "aplicacao", anchor: "middle" },
      { kind: "text", at: [4.8, 3.9], text: "desce", tone: "aplicacao", anchor: "middle" },
    ],
  },

  "componentes-de-uma-forca": {
    alt: "Seta inclinada partindo da origem, com o ângulo marcado junto ao eixo horizontal. Linhas tracejadas mostram a projeção da seta sobre cada eixo.",
    x: [-10, 110], y: [-15, 80],
    xTicks: [0, 50, 86.6], yTicks: [0, 50],
    legend: "A força se divide em duas: o cosseno dá a parte horizontal e o seno, a vertical.",
    marks: [
      { kind: "segment", from: [0, 0], to: [86.6, 50], tone: "principal" },
      { kind: "segment", from: [86.6, 0], to: [86.6, 50], tone: "aplicacao", dashed: true },
      { kind: "segment", from: [0, 50], to: [86.6, 50], tone: "ideia", dashed: true },
      { kind: "angle", at: [0, 0], from: [30, 0], to: [30, 17.3], label: "30°", tone: "neutro" },
      { kind: "point", at: [86.6, 50], tone: "principal" },
      { kind: "text", at: [52, 40], text: "100 N", tone: "principal", anchor: "middle" },
      { kind: "text", at: [43, -9], text: "Fx ≈ 86,6 N", tone: "aplicacao", anchor: "middle" },
      { kind: "text", at: [2, 57], text: "Fy = 50 N", tone: "ideia", anchor: "start" },
    ],
  },

  "rampa-altura-e-avanco": {
    alt: "Triângulo retângulo deitado representando uma rampa: a hipotenusa sobe da esquerda para a direita, com o ângulo marcado na base e os dois catetos indicados.",
    x: [-1.5, 10.5], y: [-1.8, 7],
    axes: "nenhum",
    legend: "Com 10 m de rampa a 30°, o seno dá a altura (5 m) e o cosseno dá o avanço no chão (≈ 8,66 m).",
    marks: [
      { kind: "polygon", points: [[0, 0], [8.66, 0], [8.66, 5]], tone: "principal" },
      { kind: "rightAngle", at: [8.66, 0], from: [0, 0], to: [8.66, 5], tone: "neutro" },
      { kind: "angle", at: [0, 0], from: [3, 0], to: [3, 1.73], label: "30°", tone: "alerta" },
      { kind: "text", at: [3.9, 3.1], text: "10 m", tone: "principal", anchor: "middle" },
      { kind: "text", at: [9.1, 2.5], text: "altura 5 m", tone: "aplicacao", anchor: "start" },
      { kind: "text", at: [4.33, -1.1], text: "avanço ≈ 8,66 m", tone: "ideia", anchor: "middle" },
    ],
  },

  "setor-e-comprimento-de-arco": {
    alt: "Circunferência com dois raios saindo do centro formando um ângulo agudo; o pedaço de curva entre eles está destacado.",
    x: [-4, 4.6], y: [-4, 4.4],
    axes: "nenhum",
    aspect: "igual",
    legend: "O ângulo em radianos é a razão entre o arco e o raio: com r = 3 e θ = π/3, o arco mede π.",
    marks: [
      { kind: "curve", f: (x) => Math.sqrt(Math.max(0, 9 - x * x)), from: -3, to: 3, tone: "neutro" },
      { kind: "curve", f: (x) => -Math.sqrt(Math.max(0, 9 - x * x)), from: -3, to: 3, tone: "neutro" },
      { kind: "segment", from: [0, 0], to: [3, 0], tone: "aplicacao" },
      { kind: "segment", from: [0, 0], to: [1.5, 2.6], tone: "aplicacao" },
      { kind: "curve", f: (x) => Math.sqrt(Math.max(0, 9 - x * x)), from: 1.5, to: 3, tone: "principal" },
      { kind: "angle", at: [0, 0], from: [1.2, 0], to: [0.6, 1.04], label: "60°", tone: "alerta" },
      { kind: "text", at: [1.9, -0.55], text: "r = 3", tone: "aplicacao", anchor: "middle" },
      { kind: "text", at: [3.2, 2.1], text: "arco = π", tone: "principal", anchor: "start" },
    ],
  },

  "tabela-do-limite-em-dois": {
    alt: "Tabela de duas colunas: valores de x chegando a dois por baixo e por cima, e os valores correspondentes da função, chegando a quatro.",
    x: [0, 10], y: [0, 7],
    axes: "nenhum",
    legend: "A expressão não existe em x = 2, mas a tabela mostra os dois lados convergindo para 4.",
    marks: [
      { kind: "segment", from: [0.6, 5.7], to: [7.4, 5.7], tone: "neutro" },
      { kind: "segment", from: [4, 0.6], to: [4, 6.3], tone: "neutro" },
      { kind: "text", at: [2.3, 6.05], text: "x", tone: "neutro", anchor: "middle" },
      { kind: "text", at: [5.7, 6.05], text: "(x² − 4)/(x − 2)", tone: "neutro", anchor: "middle" },
      { kind: "text", at: [2.3, 5], text: "1,9", tone: "aplicacao", anchor: "middle" },
      { kind: "text", at: [5.7, 5], text: "3,9", tone: "aplicacao", anchor: "middle" },
      { kind: "text", at: [2.3, 4], text: "1,99", tone: "aplicacao", anchor: "middle" },
      { kind: "text", at: [5.7, 4], text: "3,99", tone: "aplicacao", anchor: "middle" },
      { kind: "text", at: [2.3, 3], text: "2", tone: "alerta", anchor: "middle" },
      { kind: "text", at: [5.7, 3], text: "não existe", tone: "alerta", anchor: "middle" },
      { kind: "text", at: [2.3, 2], text: "2,01", tone: "ideia", anchor: "middle" },
      { kind: "text", at: [5.7, 2], text: "4,01", tone: "ideia", anchor: "middle" },
      { kind: "text", at: [2.3, 1], text: "2,1", tone: "ideia", anchor: "middle" },
      { kind: "text", at: [5.7, 1], text: "4,1", tone: "ideia", anchor: "middle" },
      { kind: "text", at: [8.9, 3], text: "→ 4", tone: "principal", anchor: "end" },
    ],
  },

  "variacao-media-no-intervalo": {
    alt: "Curva de posição com dois pontos marcados e um segmento entre eles. Abaixo do segmento, dois catetos formam um triângulo retângulo.",
    x: [0, 4], y: [-0.8, 10],
    xTicks: [0, 1, 2, 3], yTicks: [0, 1, 5, 9],
    legend: "De 1 s a 3 s, a posição sobe 8 m em 2 s: a variação média é 4 m/s, a inclinação do segmento.",
    marks: [
      { kind: "curve", f: (t) => t * t, from: 0, to: 3.2, tone: "neutro" },
      { kind: "segment", from: [1, 1], to: [3, 9], tone: "principal" },
      { kind: "segment", from: [1, 1], to: [3, 1], tone: "aplicacao" },
      { kind: "segment", from: [3, 1], to: [3, 9], tone: "alerta" },
      { kind: "point", at: [1, 1], tone: "principal" },
      { kind: "point", at: [3, 9], tone: "principal" },
      { kind: "text", at: [2, 0.3], text: "Δt = 2 s", tone: "aplicacao", anchor: "middle" },
      { kind: "text", at: [3.1, 5], text: "Δs = 8 m", tone: "alerta", anchor: "start" },
      { kind: "text", at: [1.15, 7], text: "média 4 m/s", tone: "principal", anchor: "start" },
    ],
  },

  "sinal-de-f-linha-na-reta": {
    alt: "Curva que sobe, desce e volta a subir. Abaixo dela, três traços no eixo horizontal marcam os trechos, com sinais de mais, menos e mais.",
    x: [-2.4, 2.4], y: [-4.2, 3.2],
    xTicks: [-2, -1, 0, 1, 2], yTicks: [-2, 0, 2],
    legend: "O sinal da derivada em cada trecho decide a direção da curva: sobe, desce, sobe.",
    marks: [
      { kind: "curve", f: (x) => x * x * x - 3 * x, from: -2.1, to: 2.1, tone: "principal" },
      { kind: "segment", from: [-2.3, -3.4], to: [-1, -3.4], tone: "aplicacao" },
      { kind: "segment", from: [-1, -3.4], to: [1, -3.4], tone: "alerta" },
      { kind: "segment", from: [1, -3.4], to: [2.3, -3.4], tone: "aplicacao" },
      { kind: "point", at: [-1, 2], tone: "neutro" },
      { kind: "point", at: [1, -2], tone: "neutro" },
      { kind: "text", at: [-1.65, -3.95], text: "f′ > 0", tone: "aplicacao", anchor: "middle" },
      { kind: "text", at: [0, -3.95], text: "f′ < 0", tone: "alerta", anchor: "middle" },
      { kind: "text", at: [1.65, -3.95], text: "f′ > 0", tone: "aplicacao", anchor: "middle" },
    ],
  },

  "producao-e-taxa-de-producao": {
    alt: "Uma curva em forma de morro representa a produção acumulada; uma reta descendente, tracejada, representa a taxa de produção e cruza o zero no topo do morro.",
    x: [0, 52], y: [-60, 700],
    xTicks: [0, 10, 25, 40, 50], yTicks: [0, 300, 625],
    legend: "A taxa de produção cai de 50 peças por hora até zero em t = 25: é exatamente onde a produção acumulada para de crescer.",
    marks: [
      { kind: "hline", at: 0, tone: "neutro" },
      { kind: "curve", f: (t) => 50 * t - t * t, from: 0, to: 52, tone: "principal" },
      { kind: "curve", f: (t) => 50 - 2 * t, from: 0, to: 52, tone: "aplicacao", dashed: true },
      { kind: "point", at: [25, 625], tone: "alerta" },
      { kind: "point", at: [25, 0], tone: "aplicacao" },
      { kind: "text", at: [25, 665], text: "produção máxima", tone: "alerta", anchor: "middle" },
      { kind: "text", at: [44, 90], text: "taxa Q′(t)", tone: "aplicacao", anchor: "middle" },
    ],
  },

  "volume-da-caixa-maximo": {
    alt: "Curva que sobe a partir da origem, atinge um topo e desce de volta ao eixo, com o ponto mais alto destacado.",
    x: [0, 6.4], y: [0, 150],
    xTicks: [0, 2, 4, 6], yTicks: [0, 50, 100, 128],
    legend: "Cortar quadrados de 2 cm nos cantos dá o maior volume: 128 cm³. Cortar de menos ou de mais piora.",
    marks: [
      { kind: "curve", f: (x) => x * (12 - 2 * x) * (12 - 2 * x), from: 0, to: 6, tone: "principal" },
      { kind: "segment", from: [2, 0], to: [2, 128], tone: "neutro", dashed: true },
      { kind: "point", at: [2, 128], tone: "alerta" },
      { kind: "text", at: [2.3, 131], text: "x = 2 → 128 cm³", tone: "alerta", anchor: "start" },
      { kind: "text", at: [5.3, 22], text: "x = 6: caixa some", tone: "neutro", anchor: "end" },
    ],
  },

  "cerca-com-lado-no-rio": {
    alt: "Retângulo com o lado de cima apoiado numa faixa que representa o rio; os outros três lados estão marcados como cerca, com as medidas indicadas.",
    x: [-1.5, 12], y: [-2, 8.5],
    axes: "nenhum",
    legend: "Com 100 m de cerca e um lado no rio, o melhor retângulo tem 25 m de profundidade e 50 m de frente: 1.250 m².",
    marks: [
      { kind: "segment", from: [-1, 6.5], to: [11.5, 6.5], tone: "ideia" },
      { kind: "text", at: [10.9, 7.3], text: "rio", tone: "ideia", anchor: "end" },
      { kind: "polygon", points: [[1, 2], [9, 2], [9, 6.5], [1, 6.5]], tone: "principal" },
      { kind: "text", at: [5, 4.1], text: "área 1.250 m²", tone: "principal", anchor: "middle" },
      { kind: "text", at: [5, 1.1], text: "frente 50 m", tone: "aplicacao", anchor: "middle" },
      { kind: "text", at: [0.6, 4.3], text: "25 m", tone: "aplicacao", anchor: "end" },
      { kind: "text", at: [9.4, 4.3], text: "25 m", tone: "aplicacao", anchor: "start" },
    ],
  },

  "candidatos-em-intervalo-fechado": {
    alt: "Parábola com três pontos destacados: um em cada ponta do intervalo e um no ponto mais baixo, no meio.",
    x: [-0.4, 5.6], y: [0, 12],
    xTicks: [0, 1, 2, 3, 4, 5], yTicks: [1, 5, 10],
    legend: "Três candidatos, três valores: 5 na borda esquerda, 1 no ponto crítico e 10 na borda direita — o máximo está na borda.",
    marks: [
      { kind: "curve", f: (x) => x * x - 4 * x + 5, from: -0.3, to: 5.4, tone: "principal" },
      { kind: "point", at: [0, 5], tone: "aplicacao" },
      { kind: "point", at: [2, 1], tone: "alerta" },
      { kind: "point", at: [5, 10], tone: "ideia" },
      { kind: "text", at: [0.25, 5.9], text: "f(0) = 5", tone: "aplicacao", anchor: "start" },
      { kind: "text", at: [2, 2], text: "mínimo: f(2) = 1", tone: "alerta", anchor: "middle" },
      { kind: "text", at: [4.75, 11], text: "máximo: f(5) = 10", tone: "ideia", anchor: "end" },
    ],
  },

  "tvm-secante-e-tangente-paralelas": {
    alt: "Parábola com um segmento ligando dois pontos e, mais abaixo, uma reta tracejada que toca a curva num único ponto, com a mesma inclinação do segmento.",
    x: [0, 4], y: [-2, 11],
    xTicks: [0, 1, 2, 3], yTicks: [0, 1, 4, 9],
    legend: "A secante de [1, 3] tem inclinação 4; o TVM garante um ponto onde a tangente tem essa mesma inclinação — aqui, c = 2.",
    marks: [
      { kind: "curve", f: (t) => t * t, from: 0, to: 3.3, tone: "neutro" },
      { kind: "segment", from: [1, 1], to: [3, 9], tone: "principal" },
      { kind: "curve", f: (t) => 4 * t - 4, from: 0.6, to: 3.2, tone: "alerta", dashed: true },
      { kind: "point", at: [1, 1], tone: "principal" },
      { kind: "point", at: [3, 9], tone: "principal" },
      { kind: "point", at: [2, 4], tone: "alerta" },
      { kind: "text", at: [1.15, 6.4], text: "secante", tone: "principal", anchor: "start" },
      { kind: "text", at: [2.75, 2.2], text: "tangente em c = 2", tone: "alerta", anchor: "end" },
    ],
  },

  "circulo-crescendo-anel": {
    alt: "Dois círculos concêntricos quase do mesmo tamanho; a faixa fina entre eles está destacada.",
    x: [-6.5, 7.5], y: [-6.5, 6.5],
    axes: "nenhum",
    aspect: "igual",
    legend: "Em um instante curto, o raio cresce um pouquinho e a área ganha o anel: por isso dA/dt = 2πr · dr/dt.",
    marks: [
      { kind: "curve", f: (x) => Math.sqrt(Math.max(0, 25 - x * x)), from: -5, to: 5, tone: "neutro" },
      { kind: "curve", f: (x) => -Math.sqrt(Math.max(0, 25 - x * x)), from: -5, to: 5, tone: "neutro" },
      { kind: "curve", f: (x) => Math.sqrt(Math.max(0, 36 - x * x)), from: -6, to: 6, tone: "alerta" },
      { kind: "curve", f: (x) => -Math.sqrt(Math.max(0, 36 - x * x)), from: -6, to: 6, tone: "alerta" },
      { kind: "segment", from: [0, 0], to: [5, 0], tone: "aplicacao" },
      { kind: "text", at: [2.5, 0.7], text: "r = 5", tone: "aplicacao", anchor: "middle" },
      { kind: "text", at: [0, 6.5], text: "o anel é o ganho de área", tone: "alerta", anchor: "middle" },
    ],
  },

  "tangente-boa-perto-ruim-longe": {
    alt: "Curva de raiz quadrada e uma reta tracejada que a toca perto do quatro; longe desse ponto, a reta se afasta claramente da curva.",
    x: [0, 10], y: [0, 4.5],
    xTicks: [0, 2, 4, 6, 9], yTicks: [0, 1, 2, 3],
    legend: "Perto de 4 a reta e a curva quase coincidem; em x = 9 a reta já dá 3,25 contra os 3 verdadeiros.",
    marks: [
      { kind: "curve", f: (x) => Math.sqrt(x), from: 0, to: 10, tone: "principal" },
      { kind: "curve", f: (x) => 2 + (x - 4) / 4, from: 0, to: 10, tone: "alerta", dashed: true },
      { kind: "point", at: [4, 2], tone: "principal" },
      { kind: "point", at: [9, 3], tone: "principal" },
      { kind: "point", at: [9, 3.25], tone: "alerta" },
      { kind: "text", at: [4.2, 1.45], text: "encosta em (4, 2)", tone: "principal", anchor: "start" },
      { kind: "text", at: [8.8, 3.85], text: "erro de 0,25", tone: "alerta", anchor: "end" },
    ],
  },

  "familia-de-antiderivadas": {
    alt: "Quatro curvas iguais empilhadas, deslocadas verticalmente umas das outras, todas com o mesmo formato.",
    x: [-3, 3], y: [-3, 9],
    xTicks: [-2, -1, 0, 1, 2], yTicks: [-2, 0, 2, 4, 6],
    legend: "Todas têm a mesma derivada: mudar o C só sobe ou desce a curva, sem mudar a inclinação em nenhum ponto.",
    marks: [
      { kind: "curve", f: (x) => (x * x) / 2 - 2, from: -3, to: 3, tone: "neutro" },
      { kind: "curve", f: (x) => (x * x) / 2, from: -3, to: 3, tone: "principal" },
      { kind: "curve", f: (x) => (x * x) / 2 + 2, from: -3, to: 3, tone: "aplicacao" },
      { kind: "curve", f: (x) => (x * x) / 2 + 4, from: -3, to: 3, tone: "ideia" },
      { kind: "text", at: [-2.9, 7.2], text: "x²/2 + C", tone: "principal", anchor: "start" },
      { kind: "text", at: [0, -2.5], text: "mesma inclinação em cada x", tone: "neutro", anchor: "middle" },
    ],
  },
